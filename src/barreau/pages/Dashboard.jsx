...
import { StatCard, EmptyState, Notice, PageHeader, SelecteurExercice } from "../components";
...
export function Dashboard() {
  ...
  return (
    <div className="space-y-5">
      ...
      {erreur && (
        <Notice ton="rouge" className="text-sm">{erreur}</Notice>
      )}
      ...
      {(data?.discipline || data?.demandesEnAttente != null || finances?.timbresAnnee != null) && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {data?.discipline && (
          ...
          {finances?.timbresAnnee != null && (
            <StatCard label="Timbres émis" value={finances.timbresAnnee} sub={`exercice ${data?.annee ?? ""}`} accent="or" valueAccent="navy" onClick={allerVers("/timbres")} />
          )}
